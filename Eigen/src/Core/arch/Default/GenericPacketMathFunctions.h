 ...
 *
 * int N;
 * Scalar x, y, _coef[N+1];
 *
 * y = polevl<decltype(x), N>( x, _coef);
 *
 *
 *
 * DESCRIPTION:
 ...
 *            N                   0
 *
 *  The function p1evl() assumes that _coef[N] = 1.0 and is
 * omitted from the array.  Its calling arguments are
 * otherwise the same as polevl().
 *
 *
 * The Eigen implementation is templatized.  For best speed, store
 * _coef as a const array (constexpr), e.g.
 *
 * const double _coef[] = {1.0, 2.0, 3.0, ...};
 *
 */
